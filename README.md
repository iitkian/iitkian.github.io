...
- **Navigation:** Top horizontal menu bar with flat, lowercase menu links (`mainpage`, `about`, `courses`, `writings`). Do not use category grouping headers (like `HOME` or `GOODIES`) to keep navigation minimal.

---

## 🎓 Credentials Hierarchy Rules

When updating credentials in the **`about.jemdoc`** file, group them by study areas (e.g., Engineering, Data Science, Remote Sensing, Economics/Management/Law) and arrange items within each category in the following tier order:
...
3. **Push to deploy:**
   ...
   ```
4. **Clean up:** Switch back to `main` branch to resume local development:
   ```bash
   git checkout main
   ```
